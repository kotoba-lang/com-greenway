(ns greenway.main
  "Kotodama WASM entrypoint for the Greenway clean-room actor (L4) — Clojure port.
  ...
  facts (`greenway.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "greenway")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"greenway.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "greenway-compat" :tier tier :entities entities} 200])
...
(defn start! [] :greenway-compat/ready)
